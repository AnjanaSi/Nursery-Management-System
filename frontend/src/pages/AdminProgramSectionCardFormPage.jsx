...
export default function AdminProgramSectionCardFormPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const isEdit = Boolean(id);

  const [card, setCard] = useState(null);
  const [form, setForm] = useState({ title: "", description: "", ageRange: "" });
  ...
  const loadCard = async () => {
    ...
    setError("");
    try {
      const res = await getAdminProgramCard(id);
      const c = res.data;
      setCard(c);
      setForm({ title: c.title || "", description: c.description || "", ageRange: c.ageRange || "" });
    } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        const res = await updateProgramCard(id, payload, imageFile);
        const updated = res.data;
        setCard(updated);
        setForm({ title: updated.title, description: updated.description, ageRange: updated.ageRange });
        clearImage();
        setEditing(false);
        setSuccess("Card updated successfully.");
        setValidated(false);
      } else {
        await createProgramCard(payload, imageFile);
        navigate("/admin/programs/cards");
      }
    } catch (err) {
  ...
  const handleCancelEdit = () => {
    if (card) {
      setForm({ title: card.title, description: card.description, ageRange: card.ageRange });
    }
    clearImage();
    setEditing(false);
    setError("");
      ...
      <div className="admin-mgmt-header d-flex align-items-center gap-3">
        ...
        <div>
          <h5 className="fw-bold mb-0" style={{ color: "var(--mk-blue)" }}>
            {isEdit ? "Edit Program Card" : "Add New Program Card"}
          </h5>
          <small className="text-muted">
            {isEdit ? "Update card content and image" : "Create a new card for the Programs section"}
          </small>
        </div>
      </div>
        ...
        <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
          <div className="admin-detail-section mb-4">
            ...
            <div className="row g-3">
              ...
              <div className="col-md-6">
                <div className="small text-muted fw-semibold mb-1">Age Range</div>
                <div>{card.ageRange}</div>
              </div>
              <div className="col-md-6">
          ...
          <div className="d-flex gap-2">
            ...
            <button
              ...
              onClick={() => navigate("/admin/programs/cards")}
            >
              &larr; Back to List
            </button>
          </div>
            ...
            <div className="row g-3">
              ...
              <div className="col-md-6">
                ...
                <input
                  ...
                  onChange={handleChange}
                  maxLength={80}
                  placeholder="e.g. Ages 6m – 2yrs"
                  required
                />
                <div className="invalid-feedback">Age range is required.</div>
              </div>
            </div>
          </div>
