...
function HomeworkFormModal({ modalRef, editTarget, onSaved }) {
  ...
  useEffect(() => {
    if (editTarget) {
      ...
      setForm({ title: "", body: "", pinned: false, urgent: false, dueDate: "", category: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editTarget) {
        await updateContent("homework", editTarget.id, payload, newFiles);
      } else {
        await createContent("homework", payload, newFiles);
      }
      onSaved();
      closeModal();
    } catch (err) {
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title fw-bold">
              {editTarget ? "Edit Homework" : "New Homework"}
            </h5>
            ...
            <div className="modal-body">
              ...
              <div className="row mb-3">
                <div className="col-sm-6">
                  <label className="form-label fw-semibold">Due Date <span className="text-muted">(optional)</span></label>
                  <input
                    type="date"
                    className="form-control"
                    value={form.dueDate}
                ...
                <div className="col-sm-6">
                  <label className="form-label fw-semibold">Category <span className="text-muted">(optional)</span></label>
                  <input
                    className="form-control"
                    value={form.category}
              ...
              <div className="mb-3">
                <div className="form-check">
                  ...
                  />
                  <label className="form-check-label" htmlFor="hw-chk-pinned">
                    Pin to top
                  </label>
                </div>
              </div>
                    ...
                    {existingAttachments.map((att) => (
                      <div key={att.id} className="tc-attachment-row">
                        <input
                          ...
                          id={`hw-keep-${att.id}`}
                        />
                        <label htmlFor={`hw-keep-${att.id}`} className="small mb-0">
            ...
            <div className="modal-footer">
              ...
              <button type="submit" className="btn btn-primary" disabled={submitting}>
                {submitting ? (
                  ...
                  editTarget ? "Save Changes" : "Post Homework"
                )}
              </button>
            </div>
          </form>
...
function HomeworkDetailModal({ modalRef, item, onEdit, onDeleted, onUnarchived }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteContent("homework", item.id);
      closeModal();
      onDeleted(item.id);
    } catch (err) {
  ...
  const handleUnarchive = async () => {
    ...
    try {
      ...
      await updateContent("homework", item.id, payload, []);
      closeModal();
  ...
  const overdue = item && item.dueDate && isOverdue(item.dueDate) && !item.archived;
...
function ArchiveYearModal({ modalRef, levelName, onArchived }) {
  ...
  const handleArchive = async () => {
    ...
    setError("");
    try {
      await archiveYear("homework", year);
      const el = modalRef.current;
      if (el) window.bootstrap?.Modal?.getInstance(el)?.hide();
      onArchived();
    } catch (err) {
  ...
  return (
    <div className="modal fade" tabIndex={-1} ref={modalRef}>
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title fw-bold">Archive Homework by Year</h5>
            <button type="button" className="btn-close" data-bs-dismiss="modal" />
          </div>
          <div className="modal-body">
            {error && <div className="alert alert-danger py-2">{error}</div>}
            ...
            <div className="alert alert-warning py-2 mb-0">
              <strong>Warning:</strong> This will archive <strong>all</strong> homework for{" "}
              <strong>{levelName}</strong> in <strong>{year || "the selected year"}</strong>. Archived items
              are hidden from parents but remain visible to teachers under the Archived filter.
            </div>
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">
...
export default function TeacherHomeworkPage() {
  ...
  const fetchItems = useCallback(async (pg = 0) => {
    ...
    setError("");
    try {
      const res = await listContent("homework", { archived: archivedFilter, page: pg, size: 10 });
      const { content, totalPages: tp, number } = res.data.data;
      setItems(content);
      setTotalPages(tp);
      setPage(number);
    } catch (err) {
      setError(err.response?.data?.error || "Failed to load homework.");
    } finally {
      setLoading(false);
    }
  }, [archivedFilter]);
  ...
  const openDetailModal = async (id) => {
    ...
    if (el) window.bootstrap.Modal.getOrCreateInstance(el).show();
    try {
      const res = await getContentById("homework", id);
  ...
  const handleUnarchived = () => fetchItems(page);
  ...
  return (
    <div className="tc-page">
      <div className="tc-page-header">
        <div>
          <h2 className="tc-page-title">Homework</h2>
          <p className="tc-page-subtitle">Manage homework assignments for your class</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="tc-empty-state">
          ...
          <h5>No {archivedFilter ? "archived" : "active"} homework</h5>
          <p className="text-muted">
            {archivedFilter
              ? "No homework has been archived yet."
              : "Post your first homework assignment using the button above."}
          </p>
        </div>
      ) : (
        <>
          <div className="tc-card-grid">
            {items.map((item) => {
              const overdue = item.dueDate && isOverdue(item.dueDate) && !item.archived;
              return (
                <div
                  key={item.id}
                ...
                >
                  ...
                  {item.dueDate && (
                    <div className="mb-2">
                      <span className={`hw-due-badge ${overdue ? "hw-due-overdue" : "hw-due-normal"}`}>
                        Due: {formatDate(item.dueDate)} {overdue ? "· Overdue" : ""}
                      </span>
                    </div>
                  )}
                  <div className="tc-card-footer">
                    ...
                    {item.attachmentCount > 0 && (
                      <span className="tc-card-attachments">📎 {item.attachmentCount}</span>
                    )}
                  </div>
                </div>
              );
